...
from ScalesBalances import cargar_json, buscar_en_certificate_data, convertir_a_gramos, identificar_id_en_certificado_balance, extraer_cmc_fijo_proporcional, calcular_incertidumbre

from htmlTemplates import CSS_STYLES, LOGO_TITLE_HTML, get_image_base64

# Obtener la imagen de fondo en base64
background_base64 = get_image_base64("background.png")

# Aplicar los estilos CSS personalizados
st.markdown(CSS_STYLES, unsafe_allow_html=True)

# Mostrar el título con estilo degradado
st.markdown(LOGO_TITLE_HTML, unsafe_allow_html=True)

# Carga inicial de los datos JSON
...
# FINAL PROCESAMIENTO DE DATOS - INICIO APP 

def buscar_por_modelo_ui():
    st.subheader("Calcular Incertidumbre por Modelo de Equipo")
    modelo_objetivo = st.text_input("Ingrese parte del modelo del equipo:", key="modelo_objetivo")

    if modelo_objetivo:
        modelos_filtrados = {k: v for k, v in certificate_data.items() if modelo_objetivo.lower() in v.get('datasheet_info', {}).get('Model No.:', '').lower()}
        if modelos_filtrados:
            opciones_modelo = [f"{k}: {v['datasheet_info']['Description:']}" for k, v in modelos_filtrados.items()]
            modelo_seleccionado = st.selectbox("Certificados encontrados (Seleccione uno):", ['Seleccione un certificado'] + opciones_modelo, key="modelo_seleccionado")
            if modelo_seleccionado != 'Seleccione un certificado':
                certificado_seleccionado = modelo_seleccionado.split(":")[0]
                # Guarda la selección en session_state y procede sin necesidad de un botón
                st.session_state['certificado_seleccionado'] = certificado_seleccionado
                seleccionar_certificado_ui(certificado_seleccionado)
        else:
            st.warning("No se encontraron modelos que coincidan con su búsqueda.")
    else:
        st.info("Ingrese parte del modelo para comenzar la búsqueda.")

def seleccionar_certificado_ui(certificado_objetivo=None):
    st.subheader("Detalles del Certificado Seleccionado")

    if not certificado_objetivo:  # Si no se proporciona un certificado, pedir al usuario
        certificado_objetivo = st.text_input("Ingrese el ID (CertNo) del certificado objetivo:", key="certificado_objetivo")

    if certificado_objetivo and certificado_objetivo in certificate_data:
        grupo_objetivo = st.selectbox("Seleccione el grupo objetivo:", 
                                      ['Seleccione un grupo'] + sorted(list(certificate_data[certificado_objetivo]['measurements'].keys())), key="grupo_objetivo")
        if grupo_objetivo != 'Seleccione un grupo':
            nominales = sorted([m['nominal'] for m in certificate_data[certificado_objetivo]['measurements'][grupo_objetivo]], key=lambda x: float(x))
            nominal_objetivo = st.selectbox("Seleccione el valor nominal objetivo:", 
                                            ['Seleccione un valor nominal'] + nominales, key="nominal_objetivo")
            if nominal_objetivo != 'Seleccione un valor nominal':
                unidad_objetivo = st.selectbox("Seleccione la unidad de medida objetivo:", 
                                               ['Seleccione una unidad', 'g', 'kg', 'lb'], key="unidad_objetivo")
                if unidad_objetivo != 'Seleccione una unidad':
                    if st.button("Calcular Incertidumbre"):
                        procesar_calculo(certificado_objetivo, grupo_objetivo, nominal_objetivo, unidad_objetivo)
    elif certificado_objetivo:
        st.error("El ID del certificado no se encuentra en la base de datos.")


def procesar_calculo(certificado_objetivo, grupo_objetivo, nominal_objetivo, unidad_objetivo):
    grupo_objetivo_format = "_".join(grupo_objetivo.lower().split())
    meas_uncert = buscar_en_certificate_data(certificate_data, certificado_objetivo, grupo_objetivo_format, nominal_objetivo, unidad_objetivo)
    ...
    incertidumbre_total = calcular_incertidumbre(nominal_en_gramos, cmc_fijo, cmc_proporcional, float(meas_uncert))
    
    st.success(f"""
    **Grupo Objetivo:** {grupo_objetivo}\n
    **Valor Nominal Objetivo:** {nominal_objetivo} {unidad_objetivo}\n
    **Incertidumbre de Medición:** {meas_uncert}\n
    **CMC utilizado:** {cmc_string}\n
    **Incertidumbre Total:** {incertidumbre_total}
    """)
...
def app():
    st.title("Asistente de Calibración y Procesamiento de Certificados")
    opcion = st.sidebar.selectbox(
        "Elija una opción:",
        ["Buscar por Número de Certificado", "Buscar por Modelo", "Procesar Certificado PDF"]
    )

    if opcion == "Buscar por Número de Certificado":
        seleccionar_certificado_ui()
    elif opcion == "Buscar por Modelo":
        buscar_por_modelo_ui()
    elif opcion == "Procesar Certificado PDF":
        procesar_pdf_ui()

def procesar_pdf_ui():
    st.subheader("Cargar y Procesar PDF de Certificados")
    uploaded_pdf = st.file_uploader("Suba su archivo PDF aquí", type=['pdf'])
    if uploaded_pdf is not None:
        handle_uploaded_pdf(uploaded_pdf)


def handle_uploaded_pdf(uploaded_pdf):
